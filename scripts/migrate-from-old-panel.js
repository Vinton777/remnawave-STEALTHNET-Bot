...
/**
 * ═══════════════════════════════════════════════════════════════════
 *  STEALTHNET 3.0 — Скрипт миграции из старой панели (Flask/SQLAlchemy)
 * ═══════════════════════════════════════════════════════════════════
 *
 *  Что мигрирует:
...
async function migrate() {
  console.log(`
╔══════════════════════════════════════════════════════════════╗
║   STEALTHNET 3.0 — Миграция из старой панели                ║
╚══════════════════════════════════════════════════════════════╝
`);
  ...
  try {
    ...
    // ─── 4. REFERRAL LINKS ───────────────────────────────────
    logSection("4/7  Реферальные связи");
    ...
    try {
      ...
      promoCodesRes = { rows: [] };
      log("⚠️", "Таблица promo_code не найдена, пропускаю");
    }
    ...
    try {
      ...
      log("⚠️", "Таблица bot_config не найдена");
    }
    ...
    try {
      ...
      log("⚠️", "Таблица branding_setting не найдена");
    }
    ...
    try {
      ...
      log("⚠️", "Таблица trial_settings не найдена");
    }
...
`);
    ...
    log("  ", "  4. Запустить синхронизацию с Remnawave из админки");
  } catch (err) {
