...
import { remnaGetUsers, remnaGetSubscriptions, remnaGetSubscriptionTemplates, remnaGetInternalSquads, remnaGetExternalSquads, remnaGetSystemStats, remnaGetSystemStatsNodes, remnaGetNodes, remnaEnableNode, remnaDisableNode, remnaRestartNode, remnaGetUser, remnaUpdateUser, remnaRevokeUserSubscription, remnaDisableUser, remnaEnableUser, remnaResetUserTraffic, remnaRemoveUsersFromInternalSquad, isRemnaConfigured, } from "../remna/remna.client.js";
...
adminRouter.post("/remna/nodes/:uuid/enable", async (req, res) => {
    ...
    if (!parsed.success)
        return res.status(400).json({ message: "Invalid node UUID" });
...
adminRouter.get("/dashboard/stats", async (_req, res) => {
    ...
    const [clientsTotal, clientsWithRemna, paidAgg, paidLast7, paidLast30, newClientsLast7, newClientsLast30] = await Promise.all([
        prisma.client.count(),
...
adminRouter.patch("/payments/:id", asyncRoute(async (req, res) => {
    ...
    let activation = { ok: false, error: "no tariff" };
    if (payment.tariffId) {
...
adminRouter.post("/tariff-categories", async (req, res) => {
    ...
    if (!body.success)
        return res.status(400).json({ message: "Неверные данные", errors: body.error.flatten() });
...
adminRouter.patch("/tariff-categories/:id", async (req, res) => {
    ...
    if (body.data.sortOrder !== undefined)
        data.sortOrder = body.data.sortOrder;
    if (body.data.emojiKey !== undefined)
        data.emojiKey = body.data.emojiKey;
...
adminRouter.patch("/tariffs/:id", async (req, res) => {
    ...
    if (body.data.name != null)
        data.name = body.data.name;
    if (body.data.durationDays != null)
        data.durationDays = body.data.durationDays;
    if (body.data.internalSquadUuids != null)
        data.internalSquadUuids = body.data.internalSquadUuids;
    ...
    if (body.data.deviceLimit !== undefined)
        data.deviceLimit = body.data.deviceLimit ?? null;
    if (body.data.price !== undefined)
        data.price = body.data.price;
    if (body.data.currency !== undefined)
        data.currency = body.data.currency.toLowerCase();
...
adminRouter.patch("/clients/:id", async (req, res) => {
    ...
    if (body.data.preferredLang !== undefined)
        updates.preferredLang = body.data.preferredLang;
    if (body.data.preferredCurrency !== undefined)
        updates.preferredCurrency = body.data.preferredCurrency;
    if (body.data.balance !== undefined)
        updates.balance = body.data.balance;
    if (body.data.isBlocked !== undefined)
        updates.isBlocked = body.data.isBlocked;
    if (body.data.blockReason !== undefined)
        updates.blockReason = body.data.blockReason;
    if (body.data.referralPercent !== undefined)
        updates.referralPercent = body.data.referralPercent;
...
async function getClientRemnaUuid(clientId) {
    const client = await prisma.client.findUnique({
        where: { id: clientId },
...
adminRouter.get("/clients/:id/remna", async (req, res) => {
    ...
    if (!remnaUuid)
        return res.status(400).json({ message: "Клиент не привязан к Remna" });
...
adminRouter.post("/clients/:id/remna/squads/add", async (req, res) => {
    ...
    const resp = (userData?.response ?? userData);
    const currentSquads = [];
    const ais = resp?.activeInternalSquads;
    if (Array.isArray(ais)) {
        for (const s of ais) {
            const u = (s && typeof s === "object" && "uuid" in s) ? s.uuid : s;
            if (typeof u === "string")
                currentSquads.push(u);
        }
    }
