...
import { isRemnaConfigured, remnaGetUsers, remnaUpdateUser, remnaCreateUser, remnaGetUserByTelegramId, remnaGetUserByEmail, remnaGetUserByUsername, extractRemnaUuid, } from "../remna/remna.client.js";
import { getSystemConfig } from "../client/client.service.js";
const PAGE_SIZE = 100;
function extractRemnaUsers(data) {
    if (Array.isArray(data))
        return data;
    if (data && typeof data === "object") {
        const obj = data;
        if (obj.response && typeof obj.response === "object") {
            const resp = obj.response.users;
...
export async function syncFromRemna() {
    ...
    for (const c of clients) {
        ...
        try {
            ...
            if (c.telegramId != null)
                body.telegramId = parseInt(c.telegramId, 10);
            if (c.email != null)
                body.email = c.email;
...
export async function createRemnaUsersForClientsWithoutUuid() {
    const result = { created: 0, linked: 0, errors: [] };
    if (!isRemnaConfigured()) {
        result.errors.push("Remna API не настроен");
        return { ok: false, ...result };
    }
