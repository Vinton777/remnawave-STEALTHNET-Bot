...
plategaWebhooksRouter.post("/platega", async (req, res) => {
    ...
    const raw = parsed.success ? parsed.data : req.body;
    ...
    if (isSuccess && payment.status === "PENDING") {
        ...
        if (payment.tariffId) {
            ...
            if (activation.ok) {
                ...
                console.error("[Platega Webhook] Tariff activation failed", { paymentId: payment.id, error: activation.error });
            }
        }
...
//# sourceMappingURL=platega.webhooks.routes.js.map