...
export function payUrlMarkup(paymentUrl, backLabel, backStyle, emojiIds) {
    ...
    if (emojiIds?.card)
        payBtn.icon_custom_emoji_id = emojiIds.card;
    return {
        inline_keyboard: [
            [payBtn],
...
export function openSubscribePageMarkup(appUrl, backLabel, backStyle, emojiIds) {
...
export function tariffCategoryButtons(categories, backLabel, innerStyles, emojiIds) {
...
export function tariffsOfCategoryButtons(category, backLabel, innerStyles, backData = "menu:tariffs", emojiIds) {
    const rows = [];
    const tariffPay = resolveStyle(toStyle(innerStyles?.tariffPay), "success");
    const back = (backLabel && backLabel.trim()) || DEFAULT_BACK_LABEL;
    const backSty = resolveStyle(toStyle(innerStyles?.back), "danger");
...
export function tariffPayButtons(categories, backLabel, innerStyles, emojiIds) {
...
export function tariffPaymentMethodButtons(tariffId, methods, backLabel, backStyle, emojiIds, balanceLabel) {
    const back = (backLabel && backLabel.trim()) || DEFAULT_BACK_LABEL;
    const backSty = resolveStyle(toStyle(backStyle), "danger");
    const cardId = emojiIds?.card;
    const rows = [];
...
export function topupPaymentMethodButtons(amount, methods, backLabel, backStyle, emojiIds) {
    const back = (backLabel && backLabel.trim()) || DEFAULT_BACK_LABEL;
    const backSty = resolveStyle(toStyle(backStyle), "danger");
    const cardId = emojiIds?.card;
    const rows = methods.map((m) => [btn(m.label, `topup:${amount}:${m.id}`, "primary", cardId)]);
    rows.push([btn(back, "menu:topup", backSty, emojiIds?.back)]);
    return { inline_keyboard: rows };
}
export function profileButtons(backLabel, innerStyles, emojiIds) {
    const back = (backLabel && backLabel.trim()) || DEFAULT_BACK_LABEL;
